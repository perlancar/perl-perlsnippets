...
=head2 IO/FILE/1 (Slurping a file content into a string)
...
=head2 IO/FILE/2 (Slurping a file content into a string)

 my $content = do { local $/; open my $fh, "<", $filename; <$fh> };

=head2 IO/FILE/3 (Slurping a file content into array of strings)
...
=head2 IO/FILE/4 (Slurping a file content into array of strings)
...
=head2 MOD/LOAD/1 (Requiring a module by name from variable)
...
=head2 MOD/LOAD/2 (Requiring a module and importing from it)
